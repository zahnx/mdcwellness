...
export default function AnimatedDisclaimer() {
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 w-full z-50 bg-surface-container-high border-t border-surface-container/30 shadow-xl px-4 md:px-16 py-4 flex flex-col md:flex-row items-center justify-between gap-2 transform transition-transform duration-300 ease-in-out
      ${show ? "translate-y-0" : "translate-y-full"}`}
    >
      {/* Disclaimer text */}
      <p className="text-on-surface text-sm md:text-base text-center md:text-left">
      ...
      <button
        ...
        className="text-on-surface hover:text-primary transition-colors mt-2 md:mt-0"
      >
        <X size={20} />
      </button>
    </div>
